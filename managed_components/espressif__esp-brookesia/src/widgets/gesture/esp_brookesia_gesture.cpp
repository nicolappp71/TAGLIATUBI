...
bool ESP_Brookesia_Gesture::updateByNewData(void)
{
    ...
    for (int i = 0; i < ESP_BROOKESIA_GESTURE_INDICATOR_BAR_TYPE_MAX; i++) {
        ...
        // Forza opacità a 0 se è la barra in basso (l'asola)
        if (i == ESP_BROOKESIA_GESTURE_INDICATOR_BAR_TYPE_BOTTOM) {
            lv_obj_set_style_bg_opa(_indicator_bars[i].get(), 0, 0);
        } else {
        ...
        // Forza opacità a 0 anche per la parte interna dell'asola
        if (i == ESP_BROOKESIA_GESTURE_INDICATOR_BAR_TYPE_BOTTOM) {
            lv_obj_set_style_bg_opa(_indicator_bars[i].get(), 0, LV_PART_INDICATOR);
        } else {
