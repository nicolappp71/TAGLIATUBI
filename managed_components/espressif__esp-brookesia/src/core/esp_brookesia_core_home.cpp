...
ESP_Brookesia_CoreHome::ESP_Brookesia_CoreHome(ESP_Brookesia_Core &core, const ESP_Brookesia_CoreHomeData_t &data) : _core(core),
                                                                                                                     _core_data(data),
...
bool ESP_Brookesia_CoreHome::updateByNewData(void)
{
    ...
    if (_core_data.background.wallpaper_image_resource.resource != nullptr)
    {
     ...
     lv_obj_t *wallpaper = lv_img_create(_main_screen_obj.get());
    lv_img_set_src(wallpaper, _core_data.background.wallpaper_image_resource.resource);
    lv_obj_align(wallpaper, LV_ALIGN_BOTTOM_MID, 0, -150);  // Posizione
    lv_obj_set_style_img_opa(wallpaper, LV_OPA_100, 0);     // Opacità
